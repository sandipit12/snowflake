...
def deploy_command(config):
  ...
  os.environ["SNOWFLAKE_PRIVATE_KEY_PATH"]= config['snowflake-private-key-path']

  scripts_skipped = 0
  scripts_applied = 0
  change_history_table = get_change_history_table_details(config['snowflake-database'],config['change-history-table'])
  if config['create-change-history-table']:
...
def get_schemachange_config(config_file_path, root_folder, modules_folder, snowflake_account, snowflake_user, snowflake_private_key_path, snowflake_role, snowflake_warehouse, snowflake_database, snowflake_database_env, change_history_table_override, vars, create_change_history_table, autocommit, verbose, dry_run):
  ...
  if snowflake_private_key_path:
    config['snowflake-private-key-path'] = snowflake_private_key_path
  if 'snowflake-private-key-path' not in config:
    config['snowflake-private-key-path'] = None
  ...
  if snowflake_database_env:
    config['snowflake-database-env'] = snowflake_database_env
  if 'snowflake-database-env' not in config:
    config['snowflake-database-env'] = None

  if snowflake_database:
    if config['snowflake-database-env']:
      config['snowflake-database'] = config['snowflake-database-env'] + '_' + snowflake_database
    if not config['snowflake-database-env']: 
  ...
  print(config['snowflake-database'])
  return config
...
def get_change_history_table_details(snowflake_database,change_history_table_override):
  ...
  details = dict()
  details['database_name'] = snowflake_database.upper() if snowflake_database is not None else _metadata_database_name.upper()
...
def create_change_history_table_if_missing(change_history_table, snowflake_session_parameters, autocommit, verbose):
  # Create the database if it doesn't exist
    query = "CREATE DATABASE IF NOT EXISTS {0}".format(
       change_history_table['database_name'])
    print(query)
    execute_snowflake_query('', query, snowflake_session_parameters, autocommit, verbose)

    # Create the schema if it doesn't exist
    query = "CREATE SCHEMA IF NOT EXISTS {0}".format(
        change_history_table['schema_name'])
    execute_snowflake_query(
        change_history_table['database_name'], query, snowflake_session_parameters, autocommit, verbose)
...
def main(argv=sys.argv):
  ...
  parser_deploy.add_argument('-pk', '--snowflake-private-key-path', type=str,help='The fully qualified name for the public key to authenticate the snowflake user', required= False)  
  ...
  parser_deploy.add_argument('--snowflake-database-env', type = str, help = 'The name of the default database environment prefix to use. Can be overridden in the change scripts.', required = False)
  ...
  if args.subcommand == 'render':
    ...
    config = get_schemachange_config(config_file_path, args.root_folder, args.modules_folder, args.snowflake_account, args.snowflake_user, args.snowflake_private_key_path, args.snowflake_role, args.snowflake_warehouse, args.snowflake_database, args.snowflake_database_env, args.change_history_table, args.vars, args.create_change_history_table, args.autocommit, args.verbose, args.dry_run)
