...
GO
--select top 10 * from Quote.Log.QuoteLogs;
...
-- select * from [Batch].[Backfill] where TableNameFQN='Quote.Log.QuoteLogs';
DECLARE
	...
	SET @SCHEMA_NAME='Log'
	SET @TABLE_NAME='QuoteLogs'
	SET @PreviousWaterMark=6719916 -- Got this from Snowflake live table using MIN(ID) From table.
