...
GO
-- select top 10 * from Quote.quote.creditScores;
...
-- select * from [Batch].[Backfill] where TableNameFQN='Quote.Quote.CreditScores';
DECLARE
	...
	SET @TABLE_NAME='CreditScores'
	SET @PreviousWaterMark=4429991 -- Got this from Snowflake live table using MIN(ID) From table.
