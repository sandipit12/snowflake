...
GO
-- select top 10 * from Quote.quote.PricingDetails;
...
-- select * from [Batch].[Backfill] where TableNameFQN='Quote.Quote.PricingDetails';
DECLARE
	...
	SELECT	*
	FROM	[Quote].[Quote].[PricingDetails]
	WHERE	
		Id >= @FromId 
		AND Id <= @ToId
	;'
	...
	SET @TABLE_NAME='PricingDetails'
	SET @PreviousWaterMark=4430002 -- Got this from Snowflake live table using MIN(ID) From table.
