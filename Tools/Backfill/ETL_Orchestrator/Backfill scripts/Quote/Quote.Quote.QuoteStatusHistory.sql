...
GO
-- select top 10 * from Quote.quote.QuoteStatusHistory;
...
-- select * from [Batch].[Backfill] where TableNameFQN='Quote.Quote.QuoteStatusHistory';
DECLARE
	...
	SELECT	*
	FROM	[Quote].[Quote].[QuoteStatusHistory]
	WHERE	
		Id >= @FromId 
		AND Id <= @ToId
	;'
	...
	SET @TABLE_NAME='QuoteStatusHistory'
	SET @PreviousWaterMark=14741137 -- Got this from Snowflake live table using MIN(ID) From table.
