...
DECLARE
	...
	,@SCHEMA_NAME SYSNAME
	,@TABLE_NAME SYSNAME
	,@TABLE_ID_COLUMN_NAME VARCHAR(20)='EntryId' /* The name of the clustered key column, default Id*/
	...
	,@CUSTOM_QUERY NVARCHAR(MAX) =N'SELECT 
		   [EntryId]
		  ,[Requestor]
		  ,[CreateDate]
		  ,[ProviderId]
		  ...
		  ,[ReferenceID]
		  ,[CallDate]
		  ,CONVERT(VARCHAR(MAX), DECOMPRESS([InputDataRawCompressed])) AS InputDataRawDeCompressed
		  ,CONVERT(VARCHAR(MAX), DECOMPRESS([OutputDataRawCompressed])) AS OutputDataRawDeCompressed
	  FROM [RSLogs].[dbo].[DataProviderRequestLogs]
	  WHERE	
		EntryId >= @FromId 
		AND EntryId <= @ToId
	;'
	...
	SET @DATABASE_NAME='RsLogs'
	SET @SCHEMA_NAME='dbo'
	SET @TABLE_NAME='DataProviderRequestLogs'
	SET @PreviousWaterMark=21569003 -- Got this from Snowflake live table using MIN(ID) From table.
	SET @Batch_size=500000 -- This is different in each table, experiment as long as the number of records is <=500 in backfill table.
	...
	BEGIN
		...
		IF (SELECT ISNULL(MIN(TransactionDate),GETDATE()) FROM [Batch].[Backfill] WHERE DatabaseName=@DATABASE_NAME and TableName=@TABLE_NAME)  < '2012-01-01'
		BEGIN
			SET @Break_flag=1;
			PRINT ('Break loop')
		END
