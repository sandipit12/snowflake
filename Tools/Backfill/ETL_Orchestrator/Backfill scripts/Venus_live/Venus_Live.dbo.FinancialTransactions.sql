...
DECLARE
	...
	,@CUSTOM_QUERY NVARCHAR(MAX) =N'SELECT *  FROM [Venus_Live].[dbo].[FinancialTransactions]
	  WHERE	
		Id >= @FromId 
		AND Id <= @ToId
	;'
	...
	SET @DATABASE_NAME='Venus_live'
	SET @SCHEMA_NAME='dbo'
	SET @TABLE_NAME='FinancialTransactions'
	SET @PreviousWaterMark=1258165505 -- Got this from Snowflake live table using MIN(ID) From table.
